...
export default function Header({ settings }: HeaderProps) {
  ...
  const { user, isAuthenticated, isAdmin, isSalesManager, isAccounts, logout, isHydrated } = useAuth();
                ...
                {isUserMenuOpen && (
                  <div className="absolute right-0 mt-3 w-56 rounded-2xl shadow-2xl bg-white/95 backdrop-blur-md border border-gray-200/50 z-50 overflow-hidden">
                    <div className="py-2">
                      ...
                      {isAccounts && (
                        <Link href="/accounts" className="flex items-center px-4 py-3 text-sm text-emerald-700 hover:bg-gradient-to-r hover:from-emerald-50 hover:to-emerald-100 transition-all duration-300 group">
                          <svg className="h-4 w-4 mr-3 text-emerald-500 group-hover:text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="border-t border-gray-200 pt-4 pb-3">
                  ...
                  <div className="mt-3 space-y-1">
                    ...
                    {isAccounts && (
                      <Link
                        href="/accounts"
                        className="block px-4 py-3 text-base font-medium text-emerald-700 hover:text-emerald-900 hover:bg-emerald-50 rounded-md touch-manipulation"
                        onClick={() => setIsMenuOpen(false)}
                      >
                        Accounts Dashboard
                      </Link>
