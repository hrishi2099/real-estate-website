...
export async function GET(request: NextRequest) {
  try {
    ...
    const performanceMetrics = salesManagerPerformance.map((manager: any) => {
      ...
      const cancelledAssignments = assignments.filter((a: any) => a.status === 'CANCELLED').length;

      // Calculate average lead score
      const leadScores = assignments
        .map((a: any) => a.lead.score || 0) // Changed from lead.leadScore?.score to lead.score
      ...
      const completedWithTimes = assignments.filter((a: any) =>
        a.status === 'COMPLETED' && a.expectedCloseDate
      );
      const avgTimeToClose = completedWithTimes.length > 0
        ? completedWithTimes.reduce((sum: number, assignment: any) => {
      ...
      const gradeDistribution = assignments.reduce((acc: Record<string, number>, assignment: any) => {
    ...
    const overallStats = {
      totalDistributed: distributionStats.reduce((sum: number, stat: any) => sum + stat._count.id, 0),
      byStatus: distributionStats.reduce((acc: Record<string, number>, stat: any) => {
        acc[stat.status] = (acc[stat.status] || 0) + stat._count.id;
        return acc;
      }, {} as Record<string, number>),
      byPriority: distributionStats.reduce((acc: Record<string, number>, stat: any) => {
    ...
    const locationAnalysis = leadSources.reduce((acc: Record<string, number>, assignment: any) => {
      try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        topLeads: topLeads.map((assignment: any) => ({
